...
int incomingByte, incomingInt;      // variables to read incoming serial data into
...
// see if there's incoming serial data:
  if (Serial.available() > 0) {
    // read the oldest integer in the serial buffer:
    incomingInt = Serial.parseInt();
    // print the read integer for debugging
    Serial.print("Read integer: ");
    Serial.print(incomingInt);

    // move the arm
    pwm.setPWM(1, 0, angleToPulses2(incomingInt));
    delay(500);
  }
